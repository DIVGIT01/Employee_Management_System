...
export const inngest = new Inngest({ id: "fullstack-ems-nt" });
...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Wait for 9 hours
    await step.sleepUntil("wait-for-the-hours", new Date(new Date().getTime() + 9 * 60 * 1000))
    ...
    if(!attendance?.checkOut){
       ...
       await step.sleepUntil("wait-for-the-1-hour", new Date(new Date().getTime() + 9 * 60 * 1000))
...
const leaveApplicationReminder = inngest.createFunction(
  ...
  async({event, step}) => {
    ...
    //wait for 24 hours
    await step.sleepUntil("wait-for-the-25-hours", new Date(new Date().getTime() + 9 * 60 * 1000))
...
const attendanceReminderCron = inngest.createFunction(
  { id: "attendance-reminder-cron", triggers: [ {event: "0 0 6 * * *"}]}, 
  async({step}) => {
    ...
    const checkedInIds = await step.run("get-checked-in-ids",
        ...
        // Step 5: Filter absent employees (not a leave & not checked in)
