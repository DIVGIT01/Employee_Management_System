...
export const createEmployees = async (req, res)=>{
    try {
        ...
        console.error("Cerate employee error:", error)
...
export const updateEmployees = async (req, res)=>{
    try {
        const {id} = req.params;
        const {firstName, lastName, email, phone, position,
            department, basicSalary, allowances, deductions, password, role, bio, employmentStatus} = req.body;
...
export const deleteEmployees = async (req, res)=>{
    try {
        ...
        const employee = await Employee.findById(id)
        if(!employee) return res.status(404).json({erorr:"Employee not found"});
        ...
        return res.json({success : true});
         ...
         return res.status(404).json({erorr:"Failed to delete employee"});
    }
}