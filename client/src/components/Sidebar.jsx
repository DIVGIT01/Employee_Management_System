...
<div className="px-5 pt-5 pb-2">
  <p className="text-[10px] text-slate-500 font-semibold uppercase tracking-[0.12em] ">Navigation</p>
</div>
...
<div className="flex-1 px-3 space-y-0.5 overflow-y-auto">
  {navItems.map((item) => {
      ...
      return(
          ...
          <Icon className={`w-[17px] h-[17px] shrink-0 ${isActive ? 'text-indigo-300' : 'text-slate-400 group-hover:text-slate-300'}`} />

          <span className="flex-1">{item.name}</span>

          {isActive &&  <ChevronRightIcon className="text-indigo-500/50 w-3.5 h-3.5 " />}
...
<div className="p-3 border-t border-white/6 ">
    <button onClick={handleLogout} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover:bg-rose-700 hover:text-rose-50 transition-all duration-150">
