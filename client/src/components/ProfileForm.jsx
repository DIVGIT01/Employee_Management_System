...
const ProfileForm = ({initialData, onSuccess}) => {
  ...
  return (
    <form onSubmit={handleSubmit} className='card p-5 sm:p-6 mb-6'>
        ...
        {error && (
            <div className='bg-rose-50 text-rose-700 p-4 rounded-xl test-sm
            border border-rose-200 mb-6 items-start gap-3'>
                <div className='w-1.5 h-1.5 rounded-full bg-rose-500 mt-1.5 shrink-0'/>
            </div>
        )}

        {message && (
            <div className='bg-emerald-50 text-emerald-700 p-4 rounded-xl test-sm
            border border-emerald-200 mb-6 items-start gap-3'>
                ...
                {initialData.isDeleted ? (
                    <div className='pt-2'>
                        <div className='p-4 bg-rose-50 border border-rose-200 rounded-xl text-center'>
                            ...
                            <p className='text-sm text-rose-500 mt-0.5'>You can no loner update your profile.</p>
                        </div>
                    </div>
                ) : (
    ...
    <button
        type="submit"
        disabled={loading}
        className="btn-primary flex items-center gap-2 justify-center px-3 py-1.5 text-xs"
    >
        {loading ? (
